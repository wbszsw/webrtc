...
for (let i = 0; i < changes.length; i++) {
	changes[i].onclick = () => {
		toggleClass(changes[0], 'big')
		toggleClass(changes[0], 'small')
		toggleClass(changes[1], 'big')
		toggleClass(changes[1], 'small')
	}
}
/***
 * socket
...
function conn() {
	console.log('conn func',io)
	//1 触发socke连接
	socket = io.connect();
	console.log('socket', socket)
	//2 加入房间后的回调
	socket.on('msg',(room,data)=> {
		console.log('文字消息:'+data)
	})
	socket.on('joined', (roomid, id) => {
		console.log('joined')
		state = 'joined';
		console.log('ceshi', roomid, id)
